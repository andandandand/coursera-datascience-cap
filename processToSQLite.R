...
twitter <- samplefile('../en_US.twitter.txt', .013)
blogs <- samplefile('../en_US.blogs.txt', .013)
news <- samplefile('../en_US.news.txt', .013)
...
# Get word frequencies in the training corpus
freq_5 <- tdmToFreq(tdm_5)
freq_4 <- tdmToFreq(tdm_4)
freq_3 <- tdmToFreq(tdm_3)
freq_2 <- tdmToFreq(tdm_2)

# Process considering previous and current word
processGram(freq_5)
processGram(freq_4)
processGram(freq_3)
processGram(freq_2)
